...
export default function BusesPage() {
  ...
  useEffect(() => {
    ...
    async function loadSegments() {
      try {
        ...
        const res = await fetch(
          `/api/fetchCVSegmentSplit?segmentName=${encodeURIComponent('bus')}`
        );
  ...
  useEffect(() => {
    ...
    async function loadAppData() {
      try {
        ...
        if (!res.ok) {
          throw new Error(`Failed to fetch bus application data: ${res.status}`);
        }
  ...
  const latestBus = pickSeries(latestPoint, ['Bus', 'bus']);
          ...
          >
            {oemError ? (
              ...
              <BarChart
                data={oemChartData}
                bars={[
                  { key: 'current', name: 'Current Period', color: '#22C55E' },
                  {
                    key: 'previous',
                    name:
          ...
          <div className="grid lg:grid-cols-2 gap-8">
            ...
            >
              {overallLoading ? (
                ...
                <LineChart overallData={overallData} category="Bus" height={300} />
              )}
            </ChartWrapper>
          </div>
...
//   const salesData = generateSalesData('buses', region);
//   const oemData = generateOEMData('buses', region, oemCompare, oemCurrentMonth);
//   const segmentData = generateSegmentData('buses', region);
//   const applicationData = generateApplicationData('buses', region, appMonth);
...
//               <span className="ml-2 font-medium text-success">+9.7% MoM</span>
//             </div>
//             <div>
//               <span className="text-muted-foreground">Leading Segment:</span>
//               <span className="ml-2 font-medium text-primary">
...
//           <ChartWrapper
//             title="Bus OEM Performance"
//             summary={`${oemData[0]?.name} dominates bus segment with ${oemData[0]?.current.toLocaleString()} units, driven by public transport expansion.`}
//             controls={
//               <div className="flex items-center space-x-3">
//                 <CompareToggle value={oemCompare} onChange={setOemCompare} />
//                 <MonthSelector 
...
//               data={oemChartData}
//               bars={[
//                 { key: 'current', name: 'Current', color: '#2ECC71' },
...
//             <ChartWrapper
//               title="Buses Segment Contribution"
//               summary={`${segmentData[0]?.name} leads with ${Math.round((segmentData[0]?.value / segmentData.reduce((sum, item) => sum + item.value, 0)) * 100)}% market share in bus segment.`}
//             >
//               <DonutChart
//                 data={segmentData}
//                 height={300}
...
//             <ChartWrapper
//               title="Buses Sales Forecast"
//               summary="Public transport modernization and school transport safety regulations drive steady growth forecast."
//             >
//               <LineChart
//                 data={forecastData.slice(-8)}
//                 lines={[
//                   { key: 'actual', name: 'Actual Sales', color: '#2ECC71' },
//                   { key: 'forecast', name: 'Forecast', color: '#007AFF', strokeDasharray: '5 5' }
//                 ]}
//                 height={300}
//                 showLegend={false}
//               />
...
//           <ChartWrapper
//             title="Buses Application Chart"
//             summary={`Public transport dominates at ${Math.round((appChartData[0]?.value / appChartData.reduce((sum, item) => sum + item.value, 0)) * 100)}% with school transport showing steady demand.`}
//             controls={
//               <MonthSelector 
//                 value={appMonth} 
...
//               bars={[{ key: 'value', name: 'Applications', color: '#2ECC71' }]}
//               height={300}
//               layout="horizontal"
//               showLegend={false}
//             />

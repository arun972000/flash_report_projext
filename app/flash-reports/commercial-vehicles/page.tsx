...
export default function CommercialVehiclesPage() {
  ...
  const latestCV = latestPoint?.data?.["CV"] ?? 0;
          ...
          >
            {oemError ? (
              ...
              <p className="text-sm text-muted-foreground">
                No commercial vehicle OEM market share data available for the
                selected period.
              </p>
            )}
          </ChartWrapper>
...
//   const salesData = generateSalesData('commercial-vehicles', region);
//   const oemData = generateOEMData('commercial-vehicles', region, oemCompare, oemCurrentMonth);
//   const segmentData = generateSegmentData('commercial-vehicles', region);
...
//               <span className="ml-2 font-medium text-success">+12.4% MoM</span>
//             </div>
//             <div>
//               <span className="text-muted-foreground">Leading Segment:</span>
//               <span className="ml-2 font-medium text-primary">
...
//         {/* Sub-categories */}
//         <div className="mb-12">
//           <h2 className="text-xl font-semibold mb-6">Commercial Vehicle Categories</h2>
...
//           <ChartWrapper
//             title="Commercial Vehicle OEM Performance"
//             summary={`${oemData[0]?.name} leads with ${oemData[0]?.current.toLocaleString()} units, benefiting from infrastructure and logistics demand.`}
//             controls={
//               <div className="flex items-center space-x-3">
//                 <CompareToggle value={oemCompare} onChange={setOemCompare} />
//                 <MonthSelector
...
//             <ChartWrapper
//               title="Commercial Vehicles Segmental Split"
//               summary={`${segmentData[0]?.name} dominates with ${Math.round((segmentData[0]?.value / segmentData.reduce((sum, item) => sum + item.value, 0)) * 100)}% market share.`}
//             >
//               <DonutChart
//                 data={segmentData}
//                 height={300}
...
//             <ChartWrapper
//               title="Commercial Vehicles Sales Forecast"
//               summary="Infrastructure growth and e-commerce expansion drive strong forecast with sustained double-digit growth expected."
//             >
//               <LineChart
...
//           <div className="grid md:grid-cols-2 gap-6 mb-8">
//             {Array.from({ length: 2 }).map((_, i) => (
//               <div key={i} className="w-full h-32 bg-muted rounded-lg shimmer"></div>
//             ))}
//           </div>
//           {Array.from({ length: 3 }).map((_, i) => (

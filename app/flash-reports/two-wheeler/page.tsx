...
export default function TwoWheelerPage() {
  ...
  const oemComputed = useMemo(() => {
    ...
    const prevMonthShort =
      monthIndex > 0 ? MONTHS_SHORT[monthIndex - 1] : "dec";
...
useEffect(() => {
    ...
    async function loadApp() {
      try {
        ...
        const res = await fetch(
          `/api/fetchAppData?segmentName=two-wheeler&segmentType=app`
        );
        if (!res.ok) {
  ...
  const latest2W = latestPoint?.data?.["2W"] ?? 0;
        ...
        <div className="space-y-8">
          ...
          >
            {overallLoading ? (
              ...
              <LineChart overallData={overallData} category="2W" height={350} />
            )}
          </ChartWrapper>
...
//               <h1 className="text-3xl font-bold mb-2">Two Wheeler Market</h1>
//               <p className="text-muted-foreground">
//                 Scooters, motorcycles, mopeds, and electric two-wheelers market analysis
//               </p>
//             </div>
...
//           <ChartWrapper
//             title="Two-Wheeler OEM Performance"
//             summary={`${oemData[0]?.name} leads with ${formatNumber(oemData[0]?.current)} units, showing ${oemData[0]?.changePercent >= 0 ? '+' : ''}${oemData[0]?.changePercent}% ${oemCompare.toUpperCase()} growth.`}
//             controls={
//               <div className="flex items-center space-x-3">
//                 <CompareToggle value={oemCompare} onChange={setOemCompare} />
//                 <MonthSelector
...
//           <ChartWrapper
//             title="Two-Wheeler EV Electric Share Comparison"
//             summary={`EV adoption increased to ${currentEV?.evShare}% from ${previousEV?.evShare}%, showing ${((currentEV?.evShare - previousEV?.evShare) || 0).toFixed(1)}pp growth in electric vehicle share.`}
//             controls={
//               <div className="flex items-center space-x-3">
//                 <CompareToggle value={evCompare} onChange={setEvCompare} />
//                 <MonthSelector
...
//           <ChartWrapper
//             title="Two-Wheeler Sales Forecast"
//             summary="Steady growth trajectory with strong festival season demand. Forecast indicates continued expansion in both urban and rural markets."
//           >
//             <LineChart
...
//             <ChartWrapper
//               title="Two-Wheeler Application Chart"
//               summary={`Personal use dominates at ${applicationData[0]?.share}% with delivery segment showing rapid expansion due to e-commerce growth.`}
//               controls={
//                 <MonthSelector
//                   value={appMonth}
//                   onChange={setAppMonth}
...
//             <ChartWrapper
//               title="Two-Wheeler Segment Distribution"
//               summary={`${segmentData[0]?.name} leads with ${Math.round((segmentData[0]?.value / segmentData.reduce((sum, item) => sum + item.value, 0)) * 100)}% market share, with scooters gaining popularity in urban markets.`}
//             >
//               <DonutChart
//                 data={segmentData}
//                 height={300}

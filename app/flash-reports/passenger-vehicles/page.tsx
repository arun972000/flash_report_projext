...
function buildCompareData(
  ...
  // Move "Others" to bottom if present
...
export default function PassengerVehiclesPage() {
  ...
  const marketSummary = useMemo(() => {
    ...
    return `${top.name} maintains leadership in passenger vehicles with ${top.current.toFixed(
      1
  ...
  const evSummary = useMemo(() => {
    ...
    return `${top.name} leads the passenger vehicle EV segment with ${top.current.toFixed(
      1
    )}% share, showing ${
      Number.isNaN(delta)
        ? "no"
  ...
  useEffect(() => {
    ...
    async function loadOverall() {
      try {
        ...
        if (!res.ok) {
          throw new Error(
            `Failed to fetch overall chart data: ${res.status}`
          );
        }
  ...
  const latestPV = pickSeries(latestPoint, ["PV", "passenger", "passenger vehicle"]);
  const prevPV = pickSeries(prevPoint, ["PV", "passenger", "passenger vehicle"]);
  const growthRate =
...
//   const salesData = generateSalesData("passenger-vehicles", region);
//   const marketData = generateOEMData(
//     "passenger-vehicles",
//     region,
//     marketCompare,
//     marketCurrentMonth
//   );
//   const evData = generateEVData(region, "passenger-vehicles");
...
//   const marketPerformanceData = marketData.slice(0, 8).map((item) => ({
//     name: item.name,
...
//   const evShareData = evData.slice(-6);
...
//         <div className="mb-8 p-6 bg-card/30 rounded-lg border border-border/50">
//           <h2 className="text-lg font-semibold mb-3">
//             Market Summary - {formatMonthForDisplay(month)}
//           </h2>
//           <div className="grid md:grid-cols-3 gap-4 text-sm">
//             <div>
...
//               <span className="ml-2 font-medium text-success">+4.6% MoM</span>
//             </div>
//             <div>
//               <span className="text-muted-foreground">EV Share:</span>
//               <span className="ml-2 font-medium text-primary">
//                 {evShareData[evShareData.length - 1]?.evShare}%
//               </span>
//             </div>
//           </div>
//         </div>
...
//               summary={`${
//                 marketData[0]?.name
//               } maintains market leadership with ${marketData[0]?.current.toLocaleString(
//                 "en-IN"
//               )} units and ${
//                 marketData[0]?.marketShare
//               }% market share. Performance shows ${
//                 marketData[0]?.changePercent >= 0 ? "+" : ""
//               }${
//                 marketData[0]?.changePercent
//               }% ${marketCompare.toUpperCase()} growth, driven by strong SUV demand and festive season promotions.`}
//               trend={marketData[0]?.changePercent >= 0 ? "up" : "down"}
//             />
//           </ChartWrapper>
...
//           >
//             <BarChart
//               data={evShareData.map((item) => ({
//                 // Use month label for the x-axis/category
//                 name: formatMonthForDisplay(item.month),
//                 evShare: item.evShare,
//                 hybridShare: item.hybridShare,
//                 // you can add totalEV here too if you want it as a third series
//                 // totalEV: item.totalEV,
//               }))}
//               bars={[
//                 {
//                   key: "evShare",
...
//                   color: "#6B7280",
//                 },
//                 // { key: 'totalEV', name: 'EV Units', color: '#007AFF' },
//               ]}
//               height={350}
//               layout="horizontal"
//             />
//             <ChartSummary
//               summary={`EV penetration in passenger vehicles reached ${
//                 evShareData[evShareData.length - 1]?.evShare
//               }% with accelerating adoption in luxury and mid-range segments. Hybrid technology at ${
//                 evShareData[evShareData.length - 1]?.hybridShare
//               }%, offering practical alternative during EV infrastructure development.`}
//               trend="up"
//             />
//           </ChartWrapper>

//           <ChartWrapper title="Passenger Vehicle Sales Forecast">
//             <LineChart
//               data={forecastData}
//               lines={[
//                 {
...
//             />
//             <ChartSummary
//               summary="Premium and electric segment growth drives positive forecast trajectory with SUV category leading market expansion. Strong consumer confidence and new model launches support sustained momentum through year-end."
//               trend="up"
//             />
//           </ChartWrapper>
...
//               />
//               <ChartSummary
//                 summary={`${appChartData[0]?.application} dominates at ${appChartData[0]?.share}% of passenger vehicle market. ${appChartData[1]?.application} segment growing rapidly at ${appChartData[1]?.share}%, supported by ride-hailing platform expansion and commercial fleet electrification.`}
//                 trend="flat"
//               />
//             </ChartWrapper>

//             <ChartWrapper title="Passenger Vehicle Segment Distribution">
//               <DonutChart data={segmentData} height={300} showLegend={true} />
//               <ChartSummary
...
//                     100
//                 )}% market share, with SUV segment showing strongest growth momentum driven by consumer preference shift.`}
//                 trend="up"
//               />
//             </ChartWrapper>
//           </div>

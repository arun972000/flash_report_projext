...
export default function ThreeWheelerPage() {
  ...
  const oemSummary = useMemo(() => {
    ...
    const compareLabel = oemCompare === 'mom' ? 'month-on-month' : 'year-on-year';

    return `${top.name} leads three-wheelers with ${top.current.toFixed(
      1
    )}% market share, showing ${
      Number.isNaN(delta) ? 'no' : `${delta >= 0 ? '+' : ''}${delta.toFixed(1)}%`
  ...
  const evComputed = useMemo(() => {
    ...
    const shortMonth = getShortMonthFromYyyyMm(evCurrentMonth || month);
    const monthIndex = MONTHS_SHORT.indexOf(shortMonth);
    const prevMonthShort = monthIndex > 0 ? MONTHS_SHORT[monthIndex - 1] : 'dec';
  ...
  const evSummary = useMemo(() => {
    ...
    const delta =
      evRow.deltaPct == null || Number.isNaN(evRow.deltaPct)
        ? 0
        : evRow.deltaPct;
    const compareLabel = evCompare === 'mom' ? 'month-on-month' : 'year-on-year';
  ...
  const latest3W = latestPoint?.data?.['3W'] ?? 0;
          ...
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
            <div>
              ...
              <p className="text-muted-foreground">
                Passenger auto-rickshaws, goods carriers, and e-rickshaws market analysis
              </p>
            </div>
        ...
        <div className="space-y-8">
          ...
          >
            {oemError ? (
              ...
              <p className="text-sm text-muted-foreground">
                No three-wheeler OEM market share data available for the selected period.
              </p>
            )}
          </ChartWrapper>
          ...
          >
            {overallLoading ? (
              ...
              <LineChart overallData={overallData} category="3W" height={350} />
            )}
          </ChartWrapper>
...
// import { CompareToggle } from '@/components/ui/CompareToggle';
// import { useAppContext } from '@/components/providers/Providers';
// import { generateSalesData, generateOEMData, generateEVData, generateApplicationData, generateSegmentData, formatNumber } from '@/lib/mockData';
...
//   const salesData = generateSalesData('three-wheeler', region);
//   const oemData = generateOEMData('three-wheeler', region, marketCompare);
//   const evData = generateEVData(region, 'three-wheeler');
//   const applicationData = generateApplicationData('three-wheeler', region, appMonth);
//   const segmentData = generateSegmentData('three-wheeler', region);
...
//   const marketSummaryData = oemData.slice(0, 5).map(oem => ({
//     name: oem.name,
//     current: oem.current,
//     previous: oem.previous,
//   }));

//   // EV Share comparison - current vs previous
...
//         <div className="space-y-8">
//           <ChartWrapper
//             title="Three-Wheeler Market Summary"
//             summary={`${oemData[0]?.name} leads the segment with ${formatNumber(oemData[0]?.current)} units, showing ${oemData[0]?.changePercent >= 0 ? '+' : ''}${oemData[0]?.changePercent}% ${marketCompare.toUpperCase()} growth across passenger and goods categories.`}
//             controls={
//               <div className="flex items-center space-x-3">
//                 <CompareToggle value={marketCompare} onChange={setMarketCompare} />
//                 <MonthSelector
...
//                 { key: 'previous', name: marketCompare === 'mom' ? 'Previous Month' : 'Previous Year', color: '#6B7280' }
//               ]}
//               height={350}
//               layout="horizontal"
//             />
...
//           <ChartWrapper
//             title="Three-Wheeler EV Electric Share Comparison"
//             summary={`E-rickshaw adoption surged to ${currentEV?.evShare}% from ${previousEV?.evShare}%, marking ${((currentEV?.evShare - previousEV?.evShare) || 0).toFixed(1)}pp increase driven by last-mile connectivity demand.`}
//             controls={
//               <div className="flex items-center space-x-3">
//                 <CompareToggle value={evCompare} onChange={setEvCompare} />
//                 <MonthSelector
...
//           <ChartWrapper
//             title="Three-Wheeler Sales Forecast"
//             summary="E-commerce expansion and urban mobility requirements drive positive outlook. E-rickshaw segment expected to maintain strong momentum."
//           >
//             <LineChart
...
//             <ChartWrapper
//               title="Three-Wheeler Application Chart"
//               summary={`Passenger transport leads at ${applicationData[0]?.share}% with goods transport and e-commerce delivery showing accelerated growth in tier-2 cities.`}
//               controls={
//                 <MonthSelector
//                   value={appMonth}
//                   onChange={setAppMonth}
...
//             <ChartWrapper
//               title="Three-Wheeler Segment Distribution"
//               summary={`${segmentData[0]?.name} dominates with ${Math.round((segmentData[0]?.value / segmentData.reduce((sum, item) => sum + item.value, 0)) * 100)}% market share, followed by goods carriers gaining traction in urban logistics.`}
//             >
//               <DonutChart
//                 data={segmentData}
//                 height={300}

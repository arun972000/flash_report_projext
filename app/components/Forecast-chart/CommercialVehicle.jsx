...
const CVForecast = () => {
    ...
    fetch('/api/overall')
      ...
      .then(apiData => {
        ...
        const chartData = apiData.map(item => {
          ...
          return {
            month: label,
            pastCV: isPast || isCurrent ? val : null,
            futureCV: isCurrent || !isPast ? val : null,
          };
        });
  ...
  const colorCV = '#9966FF';
  const colorForecastCV = `${colorCV}80`;
    ...
    <div style={{ position: 'relative', width: '100%', zIndex: 0 }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <div style={{ width: 24, height: 3, background: colorCV, borderRadius: 2 }} />
          <span>Historical CV</span>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <div style={{
            width: 24,
            height: 0,
            borderTop: `2px dashed ${colorForecastCV}`,
          }} />
          <span>Forecast CV</span>
        </div>
      </div>
    </div>
  );

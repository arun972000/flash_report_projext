...
const BusForeCast = () => {
    ...
    fetch('/api/overall')
      ...
      .then(apiData => {
        ...
        const chartData = apiData.map(item => {
          ...
          const val = item['bus'] || 0;

          return {
            month: label,
            pastBus: isPast || isCurrent ? val : null,
            futureBus: isCurrent || !isPast ? val : null,
          };
        });
  ...
  const colorBus = '#9966FF';
  const colorForecastBus = `${colorBus}80`;
    ...
    <div style={{ position: 'relative', width: '100%', zIndex: 0 }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <div style={{ width: 24, height: 3, background: colorBus, borderRadius: 2 }} />
          <span>Historical Bus</span>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <div style={{
            width: 24,
            height: 0,
            borderTop: `2px dashed ${colorForecastBus}`,
          }} />
          <span>Forecast Bus</span>
        </div>
      </div>
    </div>
  );
